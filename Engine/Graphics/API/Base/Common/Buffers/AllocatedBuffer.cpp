                                 ...
                                 const VkBufferUsageFlags inUsageFlags)
{
	...
	vmaMapMemory(gGraphics->GetAllocator(), allocation, &data);
	memcpy(data, inData, inBufferSize);
	vmaUnmapMemory(gGraphics->GetAllocator(), allocation);
}
...
void AllocatedBuffer::Create(const VkDeviceSize aSize, const VkBufferUsageFlags aUsage)
{
	...
	if (const VkResult result = vmaCreateBuffer(gGraphics->GetAllocator(), &bufferInfo, &vmaallocInfo,
	                                            &mBuffer,
...
void AllocatedBuffer::Destroy()
{
    ...
    if (VmaAllocator allocator = gGraphics->GetAllocator())
    {
        vmaDestroyBuffer(allocator, mBuffer, allocation);
    }
    else
    {
